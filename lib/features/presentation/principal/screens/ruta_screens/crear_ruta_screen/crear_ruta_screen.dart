...
class CrearRutaScreen extends StatelessWidget {
  ...
  const CrearRutaScreen({super.key, this.rutId});

  @override
  Widget build(BuildContext context) {
    final String titulo = rutId != null && rutId! > 0
        ? "Editar Ruta"
        : "Crear Ruta";
    ...
    return EstructuraBase(
      ...
      vista: CrearRutaView(rutId: rutId),
    );
  }
}
...
class CrearRutaView extends ConsumerStatefulWidget {
  final int? rutId;
  const CrearRutaView({super.key, this.rutId});
...
class _CrearRutaViewState extends ConsumerState<CrearRutaView> {
  ...
  void _validarYGuardarRuta() {
    if (_formKey.currentState!.validate()) {
      final notificador = ref.read(
        crearRutaScreenProvider(widget.rutId).notifier,
      );
      notificador.onGuardarRuta().ejecutarConLoading();
    }
  }

  @override
  Widget build(BuildContext context) {
    final estado = ref.watch(crearRutaScreenProvider(widget.rutId));
    final notificador = ref.read(
      crearRutaScreenProvider(widget.rutId).notifier,
    );

    final bool esEdicion = estado.rutId > 0;

    ref.listen(
      crearRutaScreenProvider(widget.rutId).select((s) => s.mensajeUi),
      (_, next) {
    ...
    ref.listen(
      crearRutaScreenProvider(widget.rutId).select((s) => s.eventoUI),
      (_, next) {
        if (next != null) {
    ...
    return dataAsync.when(
      ...
      data: (data) {
        ...
        if (!esEdicion && estado.venId == 0 && vendedores.isNotEmpty) {
          Future.microtask(() {
